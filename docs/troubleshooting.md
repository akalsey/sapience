...
`--fix --only <finding-id>` applies a single finding — `openclaw sapience doctor --fix --only tools:goal-collision` denies core's goal tools without touching anything else. `install.sh` uses this so a fix you accept at one prompt doesn't drag in one you declined at another.

Denying core's goal tools is the one fix with reach beyond the suite: it removes those tools from **every** session on the host. If the same agent does coding work and uses per-thread token budgets, decline it and rely on the tool descriptions instead.
