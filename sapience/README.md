...
A proposal is only worth logging when nothing already does the job, so the tool checks the skills this install actually has — `<workspace>/skills`, the state dir's `skills/`, and any extra roots in `skillsDirs` — before it writes anything. A proposal naming an installed skill is refused outright. A proposal that merely overlaps one is refused once, with the closest skills named; to log it anyway the agent must re-call with `not_covered_by` saying what the existing skill can't do, and that answer is recorded in the spec. Refusals emit `skill_proposal_duplicate_blocked` — a run of them on one name means the agent keeps rediscovering a skill it isn't using.
